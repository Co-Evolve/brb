...
from brb.toy_example.morphology.specification.specification import ToyExampleJointSpecification, \
    ToyExampleMorphologySpecification
from brb.utils import colors


class MJCToyExampleArmSegment(MJCMorphologyPart):
    def __init__(
            self,
    ...
    def _build(
        ...
        self._segment_specification = self._arm_specification.segment_specifications[segment_index]
    ...
    def _configure_joint_sensor(
            self,
            joint: mjcf.Element
            ) -> None:
        self.mjcf_model.sensor.add(
                "jointpos", joint=joint, name=f"{joint.name}"
                                              f"_jointpos_sensor"
                )
        self.mjcf_model.sensor.add(
                "jointvel", joint=joint, name=f"{joint.name}"
                                              f"_jointvel_sensor"
                )
