...
MESH_NAME_TO_MASS = {
    # in kilograms
    "ventral_dextral.stl": 0.00898,
    "ventral_sinistral.stl": 0.00901,
    "dorsal_dextral_even.stl": 0.00821,
    "dorsal_sinistral_even.stl": 0.00883,
    "dorsal_dextral_odd.stl": 0.00895,
    "dorsal_sinistral_odd.stl": 0.00926,
    "ball_bearing.stl": 0.00214,
    "vertebrae.stl": 0.00453,
    "connector_plate.stl": 0.00014,
...
def default_mesh_specification(*, mesh_name: str) -> MeshSpecification:
    return MeshSpecification(
        mesh_path=str(BASE_MESH_PATH / mesh_name),
        scale_ratio=0.001 * np.ones(3),
        mass=MESH_NAME_TO_MASS[mesh_name]
    )


def default_gliding_joint_specification(*, plate_index: int, axis: str) -> JointSpecification:
    if axis == 'x':
...
def default_seahorse_plate_specification(*, segment_index: int, plate_index: int) -> SeahorsePlateSpecification:
...
def default_seahorse_segment_specification(*, segment_index: int) -> SeahorseSegmentSpecification:
    vertebrae_specification = default_seahorse_vertebrae_specification()
    plate_specifications = [
        default_seahorse_plate_specification(segment_index=segment_index, plate_index=plate_index)
...
def default_seahorse_morphology_specification(*, num_segments: int) -> SeahorseMorphologySpecification:
    segment_specifications = [
        default_seahorse_segment_specification(segment_index=segment_index) for segment_index in range(num_segments)
    ]
    ...
    morphology_specification = SeahorseMorphologySpecification(
        ...
        tendon_actuation_specification=tendon_actuation_specification
    )
