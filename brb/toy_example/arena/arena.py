...
from mujoco_utils.arena import ArenaConfiguration, MJCFArena
from transforms3d.euler import euler2quat

from brb.toy_example.arena.target import Target
...
class MJCFPlaneWithTargetArena(MJCFArena):
    ...
    def _build_floor(
        ...
        self._ground_material = self.mjcf_model.asset.add(
                ...
                texuniform=True, reflectance=0.2, texture=self._ground_texture
                )
        # Build groundplane.
    ...
    def _configure_top_camera(
        ...
        self._main_camera = self.mjcf_body.add(
                'camera', name='main_camera', pos=[0, 0, 10], quat=euler2quat(0, 0, 0), )

    def _attach_target(
            self
            ) -> Target:
        target = Target(parent=self, name="target")
        return target
